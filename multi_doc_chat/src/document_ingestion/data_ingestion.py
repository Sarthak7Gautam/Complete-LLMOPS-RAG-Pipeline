...
"""The "Saving" code is generous because storage is cheap and easy. The "Loading" code is strict because you only want to pass data to your AI that you have a proper parser for. If you try to use TextLoader on a .xlsx file, it will load raw binary "garbage" text, which will confuse your LLM and waste your money."""
...
class ChatIngestor:
    ...
    def _split(self, docs: List[Document], chunk_size=1000, chunk_overlap=200) -> List[Document]:
        """Splits the documents for effecient processing"""
        splitter = RecursiveCharacterTextSplitter(chunk_size=chunk_size, chunk_overlap=chunk_overlap)
        chunks = splitter.split_documents(docs)
        log.info("Documents split complete", chunks=len(chunks), chunk_size=chunk_size, overlap=chunk_overlap)
        return chunks
...
class FaissManager:
    ...
    def load_or_create(self,texts:Optional[List[str]]=None, metadatas: Optional[List[dict]] = None):
        """Load the texts if they are present or create them"""
